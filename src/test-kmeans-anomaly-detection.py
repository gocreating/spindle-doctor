...
python test-kmeans-anomaly-detection.py \
    ...
    --scope phm2012 \
    --name test-2018-05-23-phm-normalized-fft1-incremental-k-means-256 \
    --step-size 32 \
    --hidden-size 64 \
    --embedding-size 128 \
    --symbol-size 256 \
...
from models.KmeansEncDecEmbedding import Model
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        # initial_centroids = np.repeat(0.0, args.symbol_size)
        # model = Model(
        #     args.step_size,
        #     args.hidden_size,
        #     args.embedding_size,
        ...
        for context in ['sampled', 'ordered']:
            if context == 'ordered' and not args.report_roc:
                break
            ...
            for batch_idx in range(0, batch_count, args.batch_step):
                ...
                # mse的計算方式會嚴重影響測試結果
                mse = np.mean((ground_truth - restored_predictions) ** 2, axis=1)
